...
function ari = adjusted_rand_index(true_labels, pred_labels)
    % 生成列联表
    contingency = crosstab(true_labels, pred_labels);
    
    % 计算各项组合数
    nij = contingency(:);
    sum_nij = sum(nij .* (nij - 1) / 2);
    
    a_i = sum(contingency, 2);
    sum_a = sum(a_i .* (a_i - 1) / 2);
    
    b_j = sum(contingency, 1);
    sum_b = sum(b_j .* (b_j - 1) / 2);
    
    n = sum(a_i);
    C_n = n * (n - 1) / 2;
    
    % 计算分子和分母
    numerator = sum_nij - (sum_a * sum_b) / C_n;
    denominator = 0.5 * (sum_a + sum_b) - (sum_a * sum_b) / C_n;
    
    % 处理分母为零的情况
    if denominator == 0
        ari = double(numerator == 0);
    else
